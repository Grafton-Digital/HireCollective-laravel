<x-layouts.public>
    ...
    <section class="bg-cream-50 px-4 nav:px-[60px] py-6" x-data="{ filtersOpen: false }">
        ...
        <div class="nav:hidden flex items-center gap-2">
            <button @click="filtersOpen = !filtersOpen" type="button" class="flex flex-1 items-center justify-between border border-[#D0D0D0] bg-white px-4 py-3 text-sm font-medium text-black">
                <span class="flex items-center gap-2">
                    ...
                    @if(request()->hasAny(['category', 'size', 'colour', 'designer', 'price', 'occasion']))<span class="inline-flex h-5 w-5 items-center justify-center bg-black text-[10px] text-white rounded-full">{{ collect(['category', 'size', 'colour', 'designer', 'price', 'occasion'])->filter(fn($f) => request($f))->count() }}</span>@endif
                </span>
            ...
            @if(request()->hasAny(['category', 'size', 'colour', 'designer', 'price', 'occasion', 'search']))
                <a href="{{ route('boutiques.show', $boutique) }}" class="flex h-[46px] items-center justify-center border border-black px-4 text-xs font-medium tracking-[1.5px] text-black hover:bg-black hover:text-white">
                    CLEAR
                </a>
            @endif
        </div>
        ...
        <form method="GET" action="{{ route('boutiques.show', $boutique) }}" class="hidden nav:flex items-end gap-4">
            ...
            @if(request()->hasAny(['category', 'size', 'colour', 'designer', 'price', 'occasion', 'search']))
                <a href="{{ route('boutiques.show', $boutique) }}" class="flex h-10 w-[120px] shrink-0 items-center justify-center border border-black text-xs font-medium tracking-[1.5px] text-black hover:bg-black hover:text-white">
                    CLEAR
                </a>
            @endif
        </form>
        ...
        <form method="GET" action="{{ route('boutiques.show', $boutique) }}" x-show="filtersOpen" x-collapse class="nav:hidden mt-3 grid grid-cols-2 gap-3" x-data="{ dirty: false }" @change="dirty = true">
            ...
            <div class="col-span-2 mt-1">
                <button type="submit" class="h-10 w-full text-xs font-medium tracking-[1.5px] transition-colors" :class="dirty ? 'bg-black text-white hover:bg-gray-800' : 'bg-gray-200 text-gray-400 cursor-not-allowed'" :disabled="!dirty">
                    APPLY
                </button>
            </div>
        </form>
    ...
    <section class="flex items-center justify-end bg-white px-4 sm:px-[60px] py-4">
        <div class="flex items-center gap-2">
            ...
            <select name="sort" onchange="window.location.href='{{ route('boutiques.show', $boutique) }}?' + new URLSearchParams({...Object.fromEntries(new URLSearchParams(window.location.search)), sort: this.value})" class="h-9 border border-[#D0D0D0] pl-3 pr-8 text-xs text-black">
