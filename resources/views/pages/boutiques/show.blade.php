                        ...
                        @foreach(($boutique->social_links ?? []) as $platform => $handle)
                            @php
                                $url = match($platform) {
                                    ...
                                    'threads' => 'https://threads.net/' . ltrim($handle, '@'),
                                    default => '#',
                                };
                            @endphp
                            <a href="{{ $url }}" target="_blank" rel="noopener" class="text-gray-400 hover:text-gray-900 transition-colors" title="{{ ucfirst($platform === 'twitter' ? 'X' : $platform) }}">
                                @switch($platform)
                                    @case('instagram')
