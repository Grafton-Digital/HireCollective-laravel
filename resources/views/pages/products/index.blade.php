<x-layouts.public>
    ...
    <section class="bg-cream-50 px-4 nav:px-[60px] py-6" x-data="{ filtersOpen: false }">
        ...
        <div class="nav:hidden flex items-center gap-2">
            <button @click="filtersOpen = !filtersOpen" type="button" class="flex flex-1 items-center justify-between border border-[#D0D0D0] bg-white px-4 py-3 text-sm font-medium text-black">
                <span class="flex items-center gap-2">
                    ...
                    @if(request()->hasAny(['category', 'size', 'colour', 'designer', 'county', 'price', 'occasion']))<span class="inline-flex h-5 w-5 items-center justify-center bg-black text-[10px] text-white rounded-full">{{ collect(['category', 'size', 'colour', 'designer', 'county', 'price', 'occasion'])->filter(fn($f) => request($f))->count() }}</span>@endif
                </span>
            ...
            @if(request()->hasAny(['category', 'size', 'colour', 'designer', 'county', 'price', 'occasion', 'search']))
                <a href="{{ route('products.index') }}" class="flex h-[46px] items-center justify-center border border-black px-4 text-xs font-medium tracking-[1.5px] text-black hover:bg-black hover:text-white">
                    CLEAR
                </a>
            @endif
        </div>
        ...
        <form method="GET" action="{{ route('products.index') }}" x-show="filtersOpen" x-collapse class="nav:hidden mt-3 grid grid-cols-2 gap-3" x-data="{ dirty: false }" @change="dirty = true">
            ...
            <div class="flex flex-col gap-1.5">
                <label class="text-2xs font-medium tracking-[1px] uppercase text-black">&nbsp;</label>
    ...
    <section class="flex items-center justify-end bg-white px-4 sm:px-[60px] py-4">
        ...
        <div class="flex items-center gap-2">
            ...
            @if ($products->total() > 24)
                <select name="per_page" onchange="window.location.href='{{ route('products.index') }}?' + new URLSearchParams({...Object.fromEntries(new URLSearchParams(window.location.search)), per_page: this.value, page: 1})" class="h-9 border border-[#D0D0D0] pl-3 pr-8 text-xs text-black">
            ...
            @endif
            <select name="sort" onchange="window.location.href='{{ route('products.index') }}?' + new URLSearchParams({...Object.fromEntries(new URLSearchParams(window.location.search)), sort: this.value})" class="h-9 border border-[#D0D0D0] pl-3 pr-8 text-xs text-black">
