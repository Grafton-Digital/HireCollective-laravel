<x-layouts.public>
    ...
    <section class="flex flex-col md:flex-row gap-12 bg-white px-4 md:px-[60px] py-8" x-data="productAvailabilityCalendar(@js($availabilityData))">
        ...
        <div class="flex w-full md:w-[55%] flex-col gap-4">
            ...
            <div class="flex flex-row gap-4">
                ...
                <button
                    type="button"
                    x-on:click="$dispatch('open-modal', 'book-test')"
                    class="mt-4 flex h-12 w-full items-center justify-center border border-[#2C2C2C] text-[13px] font-semibold tracking-[1.5px] text-[#2C2C2C] hover:bg-black hover:text-white uppercase"
                >
                    Book a Test
                </button>
            </div>
    ...
    {{-- Book a Test Modal --}}
    <x-modal name="book-test" maxWidth="md" focusable>
        <div class="p-8" x-data="bookTestForm({{ $product->id }})" x-cloak>
            <template x-if="!submitted">
                <div>
                    <div class="flex items-center justify-between">
                        <h2 class="font-serif text-[28px] italic text-black">Book a Test</h2>
                        <button type="button" x-on:click="$dispatch('close-modal', 'book-test')" class="text-gray-400 hover:text-gray-600">
                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                            </svg>
                        </button>
            ...
            <template x-if="submitted">
                <div class="py-8 text-center">
                    ...
                    <h3 class="mt-4 font-serif text-[24px] italic text-black">Request Sent!</h3>
                    <p class="mt-2 text-[13px] text-[#666]">We'll get back to you within 24 hours.</p>
                    <button type="button" x-on:click="$dispatch('close-modal', 'book-test'); resetForm()"
                            class="mt-6 inline-flex h-10 items-center justify-center border border-gray-300 px-6 text-[13px] font-medium text-gray-700 hover:bg-gray-50">
                        CLOSE
                    </button>
                </div>
